...
if(!email || !password || !nickname){
    return res.send({
        isSuccess : false,
        code : 400,
        message:"회원 가입 입력값을 입력해주세요"
    });
}

const isValidEmail = /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i; //이메일 정규식
...
const isValidPassword = /^[A-Za-z0-9]{8,20}$/; //비밀번호 정규식 6~12자 영문 숫자 조합
...
if(nickname.length < 2 || nickname > 10){   //닉네임 형식 2~10 글자 사이
    return res.send({
        isSuccess : false,
        code : 400,
...
exports.signin = async function(req,res){
    ...
    if(!isValidUser){
        return res.send({
            isSuccess : false,
            code : 400,
            message:"데이터베이스 에러, 담당자에게 문의해주세요. "
        });
    }
